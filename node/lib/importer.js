...
var SituationGoogleDocsHTML = {
  ...
  getElementFragments: function (el) {
    ...
    function isNodeCitation(nodes, idx) {
      var el0 = nodes[idx];
      var el1 = nodes[idx + 1];
      var el2 = nodes[idx + 2];
    ...
    function getPreviousCitations() {
      return _.findWhere(_.last(fragments), { type: "citations" });
    }

    function isCitationMergeable(fragment) {
      var isWhitespace = /^\w*$/.test(fragment.content);
      return isWhitespace && getPreviousCitations();
    }

    for (var idx = 0, len = nodes.length; idx < len; idx++) {
      var fragment = {
        type: "fragment",
        content: getNodeText(nodes, idx),
        decorations: []
      };

      if (isNodeEmpty(nodes, idx)) {
        continue;
      }

      if (isNodeLink(nodes, idx)) {
        var el = $(nodes[idx]);
      ...
      if (isNodeCitation(nodes, idx)) {
        var link = $(nodes[idx + 1]);
        var endcap = nodes[idx + 2];
        ...
        endcap.data = endcap.data.replace(/^\)/, "");
        idx++;

        if (isCitationMergeable(fragment.content)) {
          getPreviousCitations().data.push(citation);
          continue;
        }
